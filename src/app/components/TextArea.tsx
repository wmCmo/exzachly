        ...
        <div className='sm:max-w-full sm:mt-0 mt-4'>
            ...
            <div className="flex justify-between items-center pt-2 gap-2">
                ...
                <div className='relative flex gap-4 bg-neutral-600 dark:bg-neutral-900 rounded-md items-center cursor-pointer ml-auto'>
                    <p onClick={() => setIsChar(false)} className={`px-4 py-2 z-10 ${isChar ? 'text-white' : 'font-bold'} transition-all`}>{dict.word}</p>
                    <p onClick={() => setIsChar(true)} className={`${locale === 'en' ? 'px-4' : 'px-6'} py-2 z-10 ${isChar ? 'font-bold' : 'text-white'} transition-all`}>{dict.char}</p>
